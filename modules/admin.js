...
function _progressCell(done,total){
  ...
  return done===0
    ...
    : '<span class="badge bg-o" style="font-size:13px">'+txt+'</span>';
}
...
function printMsMemo(selectedIds){
  ...
  subjectsWithMs.forEach((sub,i)=>{
    ...
    pages+=`<div class="memo-page"${isLast?'':' style="page-break-after:always"'}>
      ...
      <!-- &#8203; (zero-width space) หลัง .fill แต่ละตัว: ถ้าค่าที่กรอกลงท้ายด้วยตัวเลข/อังกฤษ (เช่น
           "ปฏิบัติดนตรีไทย 1") แล้วตามด้วยคำไทยติดกันไม่มีช่องว่าง เบราว์เซอร์จะไม่มีจุดตัดบรรทัดให้เลือก
           เลย (ตัวเลขชนตัวอักษรไทยไม่ใช่จุดตัดบรรทัดที่ถูกต้องตามหลัก) ทำให้ทั้งก้อนข้อความยาวๆ ที่ตามมา
           ถูกลากตกไปบรรทัดถัดไปทั้งหมด เหลือที่ว่างท้ายบรรทัดก่อนหน้าเยอะผิดปกติ — ใส่จุดตัดที่มองไม่เห็นนี้
           คั่นไว้ให้เบราว์เซอร์เลือกตัดตรงนี้ได้เมื่อจำเป็น (ไม่กระทบการป้องกันคำติดของ .tj-nowrap เลย) -->
    ...
    <style>
      ...
      *{box-sizing:border-box;}
      ...
      /* ค่าที่กรอกในเนื้อความ (ชื่อครู/วิชา/รหัสวิชา/ภาคเรียน/ปีการศึกษา) ไม่ใช้จุดไข่ปลาล้อมรอบ
         ...
         position:absolute ยืดเลยกล่องจะบวมผิดปกติเมื่อ .fill ตัวใดตัวหนึ่งตัดขึ้นบรรทัดใหม่
