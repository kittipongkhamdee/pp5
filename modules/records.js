...
function renderEplUnitsTab(){
  ...
  const grand=sumWeight+sumWeightFinal;
    ...
    <div class="cb">
      <div class="tw tw-freeze"><table>
        ...
        <tr style="display:${sumWeight!==targetBetween?'table-row':'none'}">
          <td colspan="7" style="padding:8px 14px;font-size:12px;color:var(--err-txt)">${_ico.warning} คะแนนระหว่างภาครวมจากหน่วยการเรียนรู้ (${sumWeight}) ไม่ตรงกับคะแนนก่อนกลางภาค+กลางภาค+หลังกลางภาคที่ตั้งไว้ในแท็บ <a href="javascript:void(0)" onclick="swTab('epl','epl-ratio')" style="color:inherit;text-decoration:underline;font-weight:600">"สัดส่วนคะแนน"</a> (${targetBetween}) — แก้ไขคะแนนระหว่างภาคของแต่ละหน่วยด้านบน หรือปรับโครงสร้างคะแนนในแท็บสัดส่วนคะแนนให้ตรงกัน</td>
        </tr>
...
<tr><td style="padding:4px 6px">โรงเรียน</td><td colspan="3" style="padding:4px 6px;border-bottom:1px dotted #999">${esc(schoolNameOnly(cfg.school_name))}</td></tr>
<tr><td style="padding:4px 6px">อำเภอ/เขต</td><td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(cfg.school_district||'')}</td><td style="padding:4px 6px">จังหวัด</td><td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(cfg.school_province||'')}</td></tr>
<tr><td style="padding:4px 6px">ชั้นมัธยมศึกษาปีที่</td><td style="padding:4px 6px;border-bottom:1px dotted #999">${sub.grade_level}</td><td style="padding:4px 6px">ห้อง</td><td style="padding:4px 6px;border-bottom:1px dotted #999">${+sub.room===0?'':sub.room}</td></tr>
<tr><td style="padding:4px 6px">ภาคเรียนที่</td><td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${cfg.semester||''}</td><td style="padding:4px 6px;white-space:nowrap">ปีการศึกษา</td><td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${cfg.academic_year||''}</td></tr>
<tr><td style="padding:4px 6px">รหัสวิชา</td><td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.subject_code)}</td><td style="padding:4px 6px;white-space:nowrap">รายวิชา</td><td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.subject_name)}</td></tr>
<tr><td style="padding:4px 6px">ประเภท</td><td colspan="3" style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.subject_type||'')}</td></tr>
<tr><td style="padding:4px 6px">หน่วยกิต</td><td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${sub.credits}</td><td style="padding:4px 6px;white-space:nowrap">เวลาเรียน</td><td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${sub.total_hours} ชม./ภาค (${sub.hours_per_week} ชม./สัปดาห์)</td></tr>
<tr><td style="padding:4px 6px">ครูผู้สอน</td><td colspan="3" style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.teacher_name||'')}</td></tr>
</table>
  ...
  <div style="border:1.5px solid #333;padding:12px 18px 14px;font-family:'Sarabun','TH SarabunNew',sans-serif;background:#fff;position:relative;min-height:270mm">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:10px;table-layout:fixed">
      ...
      <tr><td style="padding:4px 6px">โรงเรียน</td>
          <td colspan="3" style="padding:4px 6px;border-bottom:1px dotted #999">${esc(schoolNameOnly(cfg.school_name))}</td></tr>
      <tr><td style="padding:4px 6px">อำเภอ/เขต</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(cfg.school_district||'')}</td>
          <td style="padding:4px 6px">จังหวัด</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(cfg.school_province||'')}</td></tr>
      <tr><td style="padding:4px 6px">ชั้นมัธยมศึกษาปีที่</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999">${sub.grade_level}</td>
          <td style="padding:4px 6px">ห้อง</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999">${+sub.room===0?'':sub.room}</td></tr>
      <tr><td style="padding:4px 6px">ภาคเรียนที่</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${cfg.semester||''}</td>
          <td style="padding:4px 6px;white-space:nowrap">ปีการศึกษา</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${cfg.academic_year||''}</td></tr>
      <tr><td style="padding:4px 6px">รหัสวิชา</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.subject_code)}</td>
          <td style="padding:4px 6px;white-space:nowrap">รายวิชา</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.subject_name)}</td></tr>
      <tr><td style="padding:4px 6px">ประเภท</td>
          <td colspan="3" style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.subject_type||'')}</td></tr>
      <tr><td style="padding:4px 6px">หน่วยกิต</td>
          <td style="padding:4px 6px;border-bottom:1px dotted #999;white-space:nowrap">${sub.credits}</td>
      ...
      <tr><td style="padding:4px 6px">ครูผู้สอน</td>
          <td colspan="3" style="padding:4px 6px;border-bottom:1px dotted #999">${esc(sub.teacher_name||'')}</td></tr>
    </table>
